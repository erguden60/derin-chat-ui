...
> **📦 Current stable:** v1.0.13 &nbsp;|&nbsp; **🚀 Next milestone:** v1.1.0 — [see roadmap](#-developer-roadmap)
...
| 📦 **Lightweight** | ~31-36 KB gzipped depending on build format, with the Preact runtime bundled into the distributed widget |
| 🔒 **Secure by default** | XSS-safe markdown rendering, URL-protocol allowlisting, client-side rate limiting, backend HMAC identity verification support |
...
**Current dependency model:** Preact is bundled into the SDK output. You do **not** need to install Preact separately when using `derin-chat-ui` in Next.js, React, Vue, Svelte, or vanilla JavaScript apps. The package uses Preact internally during development/build, but `vite.config.ts` does not externalize it for the distributed SDK files. Public TypeScript declarations are also consumer-safe and do not require `preact` types to be installed.

- Bundler apps only need `derin-chat-ui`.
- CDN/no-bundler usage only needs the single UMD script shown below.
- You do not need to import a separate CSS file; styles are injected into the widget Shadow DOM.
...
**Current production bundle size (v1.0.13 build):**
...
DerinChat.init({
  ...
  nonce: 'csp-nonce-value',       // optional CSP nonce for fallback style injection
  apiUrl: 'https://api.example.com/chat',
});
...
DerinChat.init({
  features: {
    history: true,         // Persist message history                   (default: follows behavior.persistSession)
...
Use `attachments` for new integrations. It owns the configurable upload menu, attachment type list, max size, and custom renderers. The older `features.fileUpload` + `ui.fileUpload` path is still supported for backward compatibility only.

```typescript
DerinChat.init({
  attachments: {
    enabled: true,         // default: false
    maxSize: 10,           // MB, default: 10
    types: [
      {
        id: 'image',
        label: 'Image',
...
DerinChat.init({
  ui: {
    ...
    showWelcomeScreen: true, // Empty-state welcome panel         (default: true)
    zIndex: 99999,          // CSS z-index of the widget          (default: 99999)
    fontFamily: 'Inter, sans-serif', // Override widget font family
    locale: 'en-US',        // Timestamp formatting locale        (default: 'en-US')
...
Pass identity data to your backend. Use `hash` to forward a server-generated HMAC signature so your backend can verify the user identity.
...
DerinChat.init({
  user: {
    ...
    hash: 'hmac_sha256_hash_here',           // Verified by your backend
...
DerinChat.init({
  ...
  onBeforeMessageSend:(message) => message.trim(),
  ...
  onMessageEdit:      (messageId, text) => console.log('Edited:', messageId, text),
  ...
  renderCustomMessage:(message) => message.sender === 'bot' ? customMessage(message) : null,
});
```
...
{
  message: string;
  sessionId?: string;              // Included in HTTP and streaming requests
...
The distributed type declarations are verified in a clean external TypeScript project. Consumers can import the public SDK types without installing `preact`.
...
- **XSS Protection** — Markdown text is parsed with strict HTML escaping and dangerous protocol blocking (`javascript:`, `data:`, etc.)
- **URL Allowlisting** — Markdown links only allow `http://`, `https://`, and `mailto:` protocols
...
- **Backend HMAC Verification Support** — Pass a server-generated `user.hash` so your backend can verify user identity
...
npm run dev
# → Opens the UI Lab at http://localhost:3000
#   The UI Lab lives in src/pages/UiLabPage.tsx and mounts the real widget with DerinChat.init().
...
Before publishing, the package is tested as an external consumer would use it:

```bash
npm --cache /tmp/derin-npm-cache pack --ignore-scripts
```

The generated `.tgz` is installed into a clean TypeScript project outside this repository. The verification checks:

- TypeScript typecheck with public imports from `derin-chat-ui`
- no required consumer install of `preact` or `@types/preact`
- ESM default import
- CJS `require`
- UMD browser script loading through `dist/index.umd.js`
- `npm pack --dry-run` package contents

The latest clean install pass verified:

- `DerinChat.init`, `destroy`, `isActive`, and `loadMessages` resolve from the installed package
- public types such as `ChatConfig`, `Message`, `ApiResponse`, `QuickReply`, `AttachmentTypeConfig`, `FileAttachment`, `ConnectionStatus`, `ConnectionConfig`, `UnreadBadgeConfig`, and `DerinChatRenderable` resolve without Preact type dependencies
- the UMD bundle renders the widget in a real browser smoke test
...
│   ├── pages/             # UI Lab, docs, and local development pages
